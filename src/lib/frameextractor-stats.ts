export interface FrameExtractorStats {
    version: string
...
async function fetchGithubReleases(): Promise<GithubRelease[]> {
    try {
        const res = await fetch(
            "https://api.github.com/repos/nokarin-dev/FrameExtractor/releases",
            {
...
async function fetchFlathubDownloads(): Promise<number> {
    try {
        const res = await fetch(
            "https://flathub.org/api/v2/stats/xyz.nokarin.FrameExtractor",
            { next: { revalidate: 3600 } }
        )
        if (!res.ok) return 0
...
export async function getFrameExtractorStats(): Promise<FrameExtractorStats> {
